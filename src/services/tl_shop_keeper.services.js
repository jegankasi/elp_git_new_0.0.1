...
const { schema, tl_shop_keeper } = require('../configs/db.schema.table.config').doc_db_config;
...
const get = async (dbConnection, id) => {
    const doc = await db_fn.get_one_from_db(dbConnection, schema, tl_shop_keeper, { id });
    return doc;
}

const getAll = async (dbConnection) => {
    const doc = await db_fn.get_all_from_db(dbConnection, schema, tl_shop_keeper, {});
    return doc;
}


const insert = async (dbConnection, body, tokenId) => {
    await formValidation(formRequiredField.tl_shop_keeper("insert"), body);
    let data = {
        ...body,
    ...
    return await db_fn.insert_records(dbConnection, schema, tl_shop_keeper, data);
}

const update = async (dbConnection, body, tokenId) => {
    try {
        await formValidation(formRequiredField.tl_shop_keeper("update"), body);
        let data = {
            ...body,
        ...
        return await db_fn.update_records(dbConnection, schema, tl_shop_keeper, criteria, data);
    } catch (error) {
        throw error;
    }
}
...
const deleteRecord = async (dbConnection, id) => {
    try {
        ...
        await db_fn.delete_records(dbConnection, schema, tl_shop_keeper, criteria);
        return "deleted";
    } catch (err) {
        throw err;
    }
