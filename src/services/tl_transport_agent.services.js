...
const get = async (dbConnection, id) => {
    const doc = await db_fn.get_one_from_db(dbConnection, schema, tl_transport_agent, { id });
    return doc;
}

const getAll = async (dbConnection) => {
    const doc = await db_fn.get_all_from_db(dbConnection, schema, tl_transport_agent, {});
    return doc;
}
...
const insert = async (dbConnection, body, tokenId) => {
    try {
        ...
        let profile = await checkAndInsertProfile(dbConnection, body.user_id, "TPA");
