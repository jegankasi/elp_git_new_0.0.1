...
const update = async (dbConnection, body, tokenId) => {
    try {
        await formValidation(formRequiredField.tl_audit_log("update"), body);
        let data = {
            ...body,
        ...
        return await db_fn.update_records(dbConnection, schema, tl_audit_log, criteria, data);
    } catch (error) {
        throw error;
    }
}
...
const deleteRecord = async (dbConnection, id) => {
    try {
        ...
        await db_fn.delete_records(dbConnection, schema, tl_audit_log, criteria);
        return "deleted";
    } catch (err) {
        throw err;
    }
