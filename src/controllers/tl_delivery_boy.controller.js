...
router.get('/id/:id', async (req, res) => {
    try {
        const data = await tl_delivery_boy.get(req, req.params.id);
...
router.post('/', async (req, res) => {
    try {
        const data = await tl_delivery_boy.insert(req.app.get("db"), req.body);
        return reposne_utils.send_response(req, res, 200, data)
    } catch (err) {
...
router.put('/id/:id', async (req, res) => {
    try {
        const data = await tl_delivery_boy.update(req.app.get("db"), req.body);
        return reposne_utils.send_response(req, res, 200, data);
    } catch (err) {
...
router.post('/saveAll', async (req, res) => {
    try {
        const data = await tl_delivery_boy.saveAll(req.app.get("db"), req.body);
        return reposne_utils.send_response(req, res, 200, data)
    } catch (err) {
        return reposne_utils.send_response(req, res, 403, err)
    }
