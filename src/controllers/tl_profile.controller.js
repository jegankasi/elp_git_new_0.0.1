...
const tl_profile_service = require("../services/tl_profile.services");
...
router.get('/', async (req, res) => {
    try {
        const data = await tl_profile_service.getAll(req.app.get("db"), req.params.id);
...
router.post('/', async (req, res) => {
    try {
        const data = await tl_profile_service.insert(req.app.get("db"), req.body);
        return reposne_utils.send_response(req, res, 200, data)
    } catch (err) {
...
router.post('/saveAll', async (req, res) => {
    try {
        const data = await tl_profile_service.saveAll(req.app.get("db"), req.body);
        return reposne_utils.send_response(req, res, 200, data)
    } catch (err) {
